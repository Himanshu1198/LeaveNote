...
export default function HodDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchRequests = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) {
          console.log('First request subject_ids:', res.data[0].subject_ids);
          console.log('First request student_ids:', res.data[0].student_ids);
        }
    ...
    const fetchStats = async () => {
      try {
        // Calculate stats from all requests for HOD
        const res = await axios.get(`${API_BASE_URL}/attendance-requests`);
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const res = await axios.get(`${API_BASE_URL}/users/${hodUserId}`);
        setUserProfile({
          name: res.data.name || '',
  ...
  const updateUserProfile = async () => {
    try {
      ...
      const res = await axios.put(`${API_BASE_URL}/users/${hodUserId}`, userData);
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="p-3 bg-gray-50 dark:bg-gray-700 border dark:border-gray-600 rounded-md">
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                      Email: {selectedRequest.student_id.email}
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="p-3 bg-gray-50 dark:bg-gray-700 border dark:border-gray-600 rounded-md">{selectedRequest.reason}</p>
                </div>
                  ...
                  <div className="space-y-2">
                    {selectedRequest.subject_ids.map((subject) => (
                      ...
                      >
                        ...
                        <div className="text-sm text-gray-500 dark:text-gray-400">
                          Teacher: {subject.teacher_id?.name || 'N/A'}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {selectedRequest.feedbackNote && (
                  <div>
                    ...
                    <p className="p-3 bg-gray-50 dark:bg-gray-700 border dark:border-gray-600 rounded-md">{selectedRequest.feedbackNote}</p>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Textarea
                ...
                className="bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 h-24"
